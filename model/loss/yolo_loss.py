...
class YoloV3Loss(nn.Module):
    def __init__(self, anchors, strides, type, iou_threshold_loss=0.5, ):
        super(YoloV3Loss, self).__init__()
        self.__loss_type = type
    ...
    def __cal_loss_per_layer(self, p, p_d, label, bboxes, stride):
        ...
        # loss giou
        if self.__loss_type == "giou":
            giou = tools.GIOU_xywh_torch(p_d_xywh, label_xywh).unsqueeze(-1)
        elif self.__loss_type == "ciou":
            giou = tools.CIOU_xywh_torch(p_d_xywh, label_xywh).unsqueeze(-1)
        elif self.__loss_type == "piou":
            giou = tools.PIOU_xywh_torch(p_d_xywh, label_xywh).unsqueeze(-1)
        elif self.__loss_type == "diou":
            giou = tools.DIOU_xywh_torch(p_d_xywh, label_xywh).unsqueeze(-1)
        ...
        #               label_noobj_mask * BCE(input=p_conf, target=label_obj_mask)) * label_mix
